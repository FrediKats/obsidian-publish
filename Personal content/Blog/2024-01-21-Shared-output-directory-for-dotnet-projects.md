...
Рост размера солюшена сказывается на билд процесс. С увеличением количества проектов не только увеличивается время сборки, но и размер создаваемых [bin директорий](MSBuild%20artifact%20directories.md#bin%20directory). Представим ситуацию, когда есть солюшен с 300 проектами. Эти проекты могут иметь много зависимостей как внутри себя, так и от бинарных файлов вне солюшена. Если размер bin директории будет в среднем 300 мегабайт, то после компиляции получаем -90 гигабайт свободного места.
...
Следующий шаг - реализация копирования с [мьютексами](Mutex.md), ради которой всё и затевалось:
```csharp
